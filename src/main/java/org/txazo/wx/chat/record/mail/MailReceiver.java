...
public class MailReceiver {
    ...
    private static void init() {
        try {
            emailProps.load(new InputStreamReader(MailReceiver.class.getResourceAsStream("/mail.properties"), "UTF-8"));
            accountProps.load(new InputStreamReader(MailReceiver.class.getResourceAsStream("/account.properties"), "UTF-8"));
        } catch (Exception e) {
    ...
    private static void parseMessageContent(Part part, MailContent content) throws Exception {
        if (part.isMimeType("text/plain")) {
            ...
            File imageFile = new File("/Users/txazo/TxazoProject/WXChatRecord/db/images/" + md5 + ".png");
            if (!imageFile.exists()) {
    ...
    private static void storage(MailContent content) throws Exception {
        FileUtil.writeFile(content.getText(), "/Users/txazo/TxazoProject/WXChatRecord/db/record/text/" + content.getId());
        FileUtil.writeFile(content.getHtml(), "/Users/txazo/TxazoProject/WXChatRecord/db/record/html/" + content.getId());
        String text = FileUtils.readFileToString(new File("/Users/txazo/TxazoProject/WXChatRecord/db/record/meta.data"), "UTF-8");
        ...
        if (!contents.contains(content)) {
            ...
            FileUtil.writeFile(JSON.toJSONString(contents, SerializerFeature.PrettyFormat), "/Users/txazo/TxazoProject/WXChatRecord/db/record/meta.data");
        }
    }
